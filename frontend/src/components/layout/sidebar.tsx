...
function AuroraRingLogo({ theme, size = 44, uniqueId }: { theme: RoleTheme; size?: number; uniqueId: string }) {
  ...
  return (
    <div
      ...
      style={{ width: size, height: size, filter: `drop-shadow(0 0 8px ${theme.accentColor}40)` }}
    >
  ...
  return (
    ...
    >
      ...
      <div className={cn('relative flex items-center justify-center shrink-0', isLeafActive && 'scale-110')}>
        <Icon
          className={cn('h-4 w-4', level > 0 && 'h-3.5 w-3.5')}
          style={{
            color: isLeafActive ? theme.accentColor : undefined,
            filter: isLeafActive ? `drop-shadow(0 0 4px ${theme.accentColor}80)` : undefined,
          }}
        />
      </div>
