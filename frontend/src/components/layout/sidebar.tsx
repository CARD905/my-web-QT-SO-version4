...
interface SidebarProps {
  role?: string;
  /** Mobile drawer state — controlled by AppShell */
  mobileOpen?: boolean;
  onMobileClose?: () => void;
}
...
export function Sidebar({ role: initialRole, mobileOpen = false, onMobileClose }: SidebarProps) {
          ...
          {items.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                onClick={onMobileClose} // ปิด drawer เมื่อ navigate บนมือถือ
                className={cn(
  ...
  return (
    <>
      {/* ── DESKTOP sidebar (lg+) ── */}
      <aside className={cn(
        'hidden lg:flex sticky top-0 h-screen flex-col border-r bg-card/70 backdrop-blur-xl transition-all duration-300 z-20',
        collapsed ? 'w-16' : 'w-60',
      )}>
        {brandHeader(false)}
        {navContent}
      ...
      {/* ── MOBILE drawer (< lg) ── */}
      <aside className={cn(
        'fixed inset-y-0 left-0 z-40 w-72 flex flex-col border-r bg-card/95 backdrop-blur-xl transition-transform duration-300 lg:hidden',
