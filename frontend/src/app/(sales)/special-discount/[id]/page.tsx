...
export default function SpecialDiscountApprovePage() {
  ...
  const handleModify = async () => {
    ...
    if (isNaN(pct) || pct < 0 || pct > 50) { toast.error('กรุณาระบุ % ระหว่าง 0-50'); return; }
  ...
  const specialPct = (q as any).specialDiscountPercent;
  const specialReason = (q as any).specialDiscountReason;
  const specialStatus = (q as any).specialDiscountStatus;
  const finalPct = (q as any).specialDiscountFinalPct;
  const isPending = specialStatus === 'PENDING_CEO';
  ...
  const statusConfig: Record<string, { color: string; label: string; icon: React.ElementType }> = {
    ...
    MODIFIED: { color: 'border-blue-400 bg-blue-50 dark:bg-blue-900/20', label: `อนุมัติ ${finalPct}%`, icon: CheckCircle2 },
  };
  const sc = statusConfig[specialStatus] ?? statusConfig['PENDING_CEO'];
  ...
  return (
    <div className="space-y-5 max-w-3xl">
        ...
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Badge variant="outline" className="bg-amber-50 text-amber-700 border-amber-400">
              <Star className="h-3 w-3 mr-1" />{specialPct}%
            </Badge>
          </div>
      ...
      <Card className={`border-2 ${sc.color}`}>
        <CardContent className="pt-4 pb-4 flex items-center gap-3">
          ...
          <div>
            ...
            {specialStatus === 'REJECTED' && <p className="text-xs text-muted-foreground mt-0.5">ส่วนลดถูกปรับลงเหลือ 20% อัตโนมัติแล้ว</p>}
            {specialStatus === 'MODIFIED' && <p className="text-xs text-muted-foreground mt-0.5">CEO อนุมัติที่ {finalPct}% (ขอ {specialPct}%)</p>}
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="pt-6 space-y-4">
          ...
          <div className="rounded-xl border p-4 bg-muted/30">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-center">
                ...
                <div className="text-3xl font-bold text-amber-600">{specialPct}%</div>
              </div>
              <div className="text-center">
                <div className="text-xs text-muted-foreground mb-1">ผลต่าง</div>
                <div className="text-xl font-bold text-amber-500">+{Number(specialPct) - 20}%</div>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-start gap-3 p-4 rounded-xl border border-amber-200 bg-amber-50/50 dark:bg-amber-900/10">
            ...
            <div>
              ...
              <div className="text-sm text-amber-900 dark:text-amber-200">{specialReason}</div>
            </div>
          </div>
      ...
      {/* ✅ CEO Action Buttons — แสดงเฉพาะ PENDING_CEO */}
      {isPending && (
            ...
            <div className="grid grid-cols-1 gap-3">
              ...
              >
                ...
                <div>
                  ...
                  <div className="text-xs text-muted-foreground mt-0.5">ระบบจะปรับส่วนลดลงเหลือ 20% อัตโนมัติ</div>
                </div>
              </button>
              ...
              <div>
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-muted-foreground mt-0.5">กำหนด % ที่อนุมัติเอง (ระหว่าง 20-50%)</div>
                  </div>
                </button>
                  ...
                  <div className="mt-2 p-4 rounded-xl border border-amber-300 bg-amber-50/50 dark:bg-amber-900/10 space-y-3">
                    ...
                    <div className="flex gap-2">
                      <Input
                        ...
                        placeholder="เช่น 25"
                        className="w-32 border-amber-300"
                      />
                      <span className="flex items-center text-sm text-muted-foreground">%</span>
                      <Button
                        onClick={handleModify}
                        disabled={acting !== null || !modifyPct}
                        className="bg-amber-600 hover:bg-amber-700"
                        size="sm"
                      >
                        {acting === 'modify' ? <Loader2 className="h-4 w-4 animate-spin" /> : null}
                        ยืนยัน
                      </Button>
                    </div>
              ...
              >
                ...
                <div>
                  <div className="font-semibold text-emerald-700 dark:text-emerald-400">🟢 อนุมัติ {specialPct}%</div>
                  <div className="text-xs text-muted-foreground mt-0.5">อนุมัติตามที่ Sales ขอ ({specialPct}%)</div>
                </div>
              </button>
            </div>
          </CardContent>
